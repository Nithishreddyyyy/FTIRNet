...
const Reports = () => {
  ...
  const handleDownload = (report) => {
    ...
    if (report.model_used === 'Peak Identifier') {
      let peaksInfo = '';
      if (typeof report.predictions === 'object' && Array.isArray(report.predictions.peaks) && report.predictions.peaks.length > 0) {
        peaksInfo = '\n\nIdentified Peaks:\n' + report.predictions.peaks.map((p, i) => 
          `  ${i+1}. Wavenumber: ${p.wavenumber} cm⁻¹ | Group: ${p.functional_group}`
        ).join('\n');
      }

      reportText = `
SpectraVision Peak Identification Report
============================
ID: ${report.id || 'N/A'}
Sample ID: ${report.sample_id || 'N/A'}
Polymer: ${typeof report.predictions === 'object' ? report.predictions.class || 'N/A' : 'N/A'}
Peaks Detected: ${typeof report.predictions === 'object' ? report.predictions.peaks_found || 'N/A' : 'N/A'}${peaksInfo}
Date: ${report.created_at ? new Date(report.created_at).toLocaleString() : 'N/A'}
Summary: ${report.summary || 'N/A'}
============================
Generated by Microplastics AI System
      `.trim();
    } else {
      const conf = typeof report.predictions === 'object' ? report.predictions.confidence || 0 : 0;
      const formattedConf = conf <= 1 ? Math.round(conf * 10000) / 100 : Math.round(conf * 100) / 100;
...
Sample ID: ${report.sample_id || 'N/A'}
Polymer: ${typeof report.predictions === 'object' ? report.predictions.class || report.predictions.predicted_polymer : 'N/A'}
Confidence: ${formattedConf}%
Model: ${report.model_used || 'N/A'}
Date: ${report.created_at ? new Date(report.created_at).toLocaleString() : 'N/A'}
Summary: ${report.summary || 'N/A'}
============================
Generated by Microplastics AI System
    ...
    a.href = url;
    a.download = `report_${report.sample_id || report.id || 'unknown'}.txt`;
    a.click();
